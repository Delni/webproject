<?php
  class View extends MyObject {
    ...
    public function getGameList($id=NULL, $hist=false){
      if ($id==NULL) {
        $sql_req='SELECT nom, createur, prive, COUNT(Pseudo)AS nb_joueurs FROM Plateau LEFT JOIN jouer ON Plateau.Id_plat=jouer.Id_Plat GROUP BY plateau.Id_plat ORDER BY plateau.Id_plat';
      } elseif($hist) {
        $sql_req='SELECT nom, createur, prive, score FROM Plateau
                  LEFT JOIN jouer ON Plateau.Id_plat=jouer.Id_Plat
                  WHERE Pseudo="'. $id .'" GROUP BY plateau.Id_plat ORDER BY plateau.Id_plat';
      } else {
        $sql_req='SELECT nom, createur, prive, nb_joueurs FROM Plateau
                  LEFT JOIN jouer ON Plateau.Id_plat=jouer.Id_Plat
          ...
          <form class="form-horizontal" method="post" action="index.php#">
            <div class="col-lg-12">
              <div class="input-group">
                <input type="text" class="form-control mdp-ctrl" placeholder="'.$placeholder.'" required>
